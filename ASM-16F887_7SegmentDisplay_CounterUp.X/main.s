;*******************************************************************************
;*
;*   Control de display de 7 segmentos: Contador asendente 0 a 9
;*
;*******************************************************************************
;* FileName:        main.asm
;* Processor:       PIC16F887
...
;* Description:     Contador ascendente con un display de 7 segmentos 
...
MCUInit:
    ...
    BSF     TRANSISTOR4		;Set transistor = 1
    ...
    BANKSEL CounterDisplay 
    CLRF    BANKMASK(CounterDisplay)	;Clear CounterDisplay
    
    GOTO    UPDATEDisplay	;Mostramos cero en el display
				;y esperamos 0.5s para iniciar conteo

;********** C O D I G O * P R I N C I P A L ************************************
LOOP: 
    INCF    CounterDisplay,1	;Incrementamos CounterDisplay
    MOVLW   10           
    SUBWF   CounterDisplay,0    ;Revisamos que no pase de 10    
    BTFSS   STATUS,2		;Revisamos STATUSbits.Z 
    GOTO    UPDATEDisplay	;Si no es 10 mostramos digito
    GOTO    RESTARTCounter	;Si es 10 reseteamos CounterDisplay

RESTARTCounter:
    MOVLW   0
    MOVWF   CounterDisplay
    GOTO    UPDATEDisplay
    
UPDATEDisplay:
    MOVF    CounterDisplay,0    ;Pasamos ContadorDisplay a W
    CALL    TABLASegmentos	;Cargamos segmentos de la tabla
    MOVWF   PORTD		;Mostramos digitos en display
    CALL    Retardo_500ms
    GOTO    LOOP
