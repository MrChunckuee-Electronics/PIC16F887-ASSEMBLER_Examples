...
MCUInit:
    BANKSEL OSCCON
    MOVLW 0b01110001
    MOVWF ((OSCCON) and 07Fh) ;Oscilador interno 8MHz --> IRCF<2:0> = 1, CCS = 1

    BANKSEL ANSEL
    CLRF ((ANSEL) and 07Fh) ;select digital IO
    CLRF ((ANSELH) and 07Fh)

    BANKSEL TRISC
    CLRF ((TRISC) and 07Fh) ;All PORTC outputs
    BANKSEL PORTC
    CLRF ((PORTC) and 07Fh) ;Clear PORTC
    BSF PORTC, 3 ; Display 4 ;Set transistor = 1
    ;Esto no es necesario si usas solo el display

    BANKSEL TRISD
    CLRF ((TRISD) and 07Fh) ;All PORTD outputs
    BANKSEL PORTD
    CLRF ((PORTD) and 07Fh) ;Clear PORTD

    BANKSEL CounterDisplay
    CLRF ((CounterDisplay) and 07Fh) ;Clear CounterDisplay
