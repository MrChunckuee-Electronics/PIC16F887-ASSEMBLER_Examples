...
MCUInit:
    ...
    BANKSEL TRISD
    MOVLW 0x00
    MOVWF ((TRISD) and 07Fh) ;all PORTD outputs
    BANKSEL PORTD
...
ADC_Init:
    ...
    BANKSEL ADCON1
    movlw 0x0E ;Configura los canales para usar solo ((PORTA) and 07Fh), 0/AN0 y
    movwf ((ADCON1) and 07Fh) ;selecciona la justificacion a la izquierda
    BANKSEL TRISA
    movlw 0x01
    movwf ((TRISA) and 07Fh) ;Se coloca ((PORTA) and 07Fh), 0 como entrada (analoga).
    BANKSEL PORTA ;Selecciona el banco 0 nuevamente
    clrf ((PORTA) and 07Fh) ;Clear PORTA
    ...
    movwf ((ADCON0) and 07Fh) ; Canal cero del ADC (AN0) y activa el ADC.
...
Update_LED0:
    ...
    MOVLW 0b00000000
    MOVWF PORTD
    GOTO ADC_Read ; Volvemos a leer ((PORTA) and 07Fh), 0
