...
MCUInit:
    ...
    CLRF ((Unidad) and 07Fh) ; Unidad, Decena, Centena y Millar
    BANKSEL Decena
    CLRF ((Decena) and 07Fh)
    BANKSEL Centena
    CLRF ((Centena) and 07Fh)
    BANKSEL Millar
    CLRF ((Millar) and 07Fh)
...
LOOP:
    ...
    MOVLW 10
    SUBWF Unidad,0
    BTFSS STATUS,2 ; Definimos bit Z del registro STATUS
...
SHOWDisplay:
    ...
    BCF PORTC, 3 ; Display 4 ; Apago display de Millar
    MOVWF PORTD ; Cargo Unidad en PORTD
    BSF PORTC, 0 ; Display 1 ; Enciendo display de Unidad
    CALL Retardo_5ms
    MOVF Decena,0 ; Obtengo digitos de 7 segmentos para Decena
    CALL TABLA7Segmentos
    BCF PORTC, 0 ; Display 1 ; Apago display de Unidad
    MOVWF PORTD ; Cargo Decena en PORTD
    BSF PORTC, 1 ; Display 2 ; Enciendo display de Decena
    CALL Retardo_5ms
    MOVF Centena,0 ; Obtengo digitos de 7 segmentos para Centena
    CALL TABLA7Segmentos
    BCF PORTC, 1 ; Display 2 ; Apago display de Decena
    MOVWF PORTD ; Cargo Centena en PORTB
    BSF PORTC, 2 ; Display 3 ; Enciendo display Centena
    CALL Retardo_5ms
    MOVF Millar,0 ; Obtengo digitos de 7 segmentos para Millar
    CALL TABLA7Segmentos
    BCF PORTC, 2 ; Display 3 ; Apago display de Centena
    MOVWF PORTD ; Cargo Millar en PORTB
    BSF PORTC, 3 ; Display 4 ; Enciendo display Millar
    CALL Retardo_5ms
